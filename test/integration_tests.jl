@testset "Tau UV disk blob" begin
    ...
    z_range = range(0, stop=1000, length=50)
    r_d_range = range(6, stop=1000, length=50)
    phi_d_range = range(0, stop=pi, length=50)
    deltas = sqrt.(r_range.^2 + r_d_range.^2 + z_range.^2 - 2 .* r_range .* r_d_range .* cos.(phi_d_range))
    taus = deltas .* SIGMA_T .* wind.config["wind"]["n_shielding"] .* wind.bh.R_g
