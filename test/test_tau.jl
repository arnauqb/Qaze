...
wind = initialize(string(@__DIR__,"/test_tau.toml"))
...
function fill_constant_density_grid(wind, density)
    quadtree_initialize(wind)
    r_range = range(0, 256, length=1000)
    z_range = range(0, 256, length=1000)
    for r in r_range
        for z in z_range
            quadtree_fill_point([r,z], 2, density, 1, nothing, wind)
        end
    end
end

function fill_densityprofile_grid(wind, n0, r0, z0)
    quadtree_initialize(wind)
    r_range = range(0, 256, length=1000)
    z_range = range(0, 256, length=1000)
    for r in r_range
...
function tau_densityprofile_analytical_simple(rd, r, z, n0, r0, z0)
...
@testset "Test simple profile density grid" begin
    wind = initialize(string(@__DIR__,"/test_tau_simple.toml"))
    n0 = 1e8
    r0 = 10
    z0 = 1
    ...
    @showprogress 1 "Computing taus..." for rd in r_d_range
        for r in r_range
            for z in z_range
                ...
                res = isapprox(tautrue, tau, atol=0, rtol=2.5)
                if !res 
                    println("rd: $rd")
                    println("r: $r")
                    println("z: $z")
