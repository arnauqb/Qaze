...
function plot_taux(wind; xl=0, xh=3000, yl=0, yh=3000, grid=true, depth = 4, nr=250, nz=251, vmin=nothing, vmax=nothing, ax = nothing, fig=nothing)
    first = false
    if ax === nothing
        first = true
        fig, ax = PyPlot.plt.subplots()
...
function plot_tessellation(wind, ax = nothing)
    if ax === nothing
        fig , ax= plt.subplots()
    end
    #xnorm, ynorm = getplotxy(voronoiedges(wind.tessellation))
    x, y = getplotxy(voronoiedges(wind.tessellation))
    #xnorm, ynorm = getplotxy(delaunayedges(wind.tessellation))
    
    #x = (xnorm .- 1) * wind.r_max
    #y = (ynorm .- 1) * wind.z_max
    
    #xnorm, ynorm = getplotxy(voronoiedgeswithoutgenerators(wind.tessellation))
    #x, y = denormalize_point.(xnorm, ynorm, Ref(wind))
    ax.plot(x, y)
    #'ax.set_xlim(0.0 ,wind.r_max)
    #'ax.set_ylim(0.0 ,wind.z_max)
    #ax.set_xlim(0,wind.r_max)
    #ax.set_ylim(0, wind.z_max)
    return fig, ax
end

function plot_line_normalized(line, wind, ax)
    pointsnorm = normalize_point.(line.p.u_hist[:,1], line.p.u_hist[:,2], Ref(wind))
    pointsnorm = hcat(pointsnorm...)
    ax.plot(pointsnorm[1,:], pointsnorm[2,:], "o-", color="black")
end